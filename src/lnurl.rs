...
pub fn make_lnurl(base_url: &str, atm_secret: &str, amount: u64) -> String {
    ...
    let encoded = URL_SAFE.encode(&salted);

    // Build full URL
    let prepared_url = format!("{}?atm=1&p={}", base_url, encoded);
...
mod tests {
    ...
    fn test_derive_key_and_iv_empty_secret() {
        ...
        assert_ne!(
            key, [0u8; 32],
            "Key should not be all zeros with empty secret"
        );
    }

    #[test]
    fn test_derive_key_and_iv_known_vector() {
        // Verified against: echo -n "MySecret" | openssl enc -aes-256-cbc -S 0102030405060708 -P -md md5
        let secret = "MySecret";
        ...
        let expected_key: [u8; 32] = [
            0x7F, 0x8A, 0x47, 0x62, 0x59, 0x34, 0x48, 0x68, 0x04, 0xA8, 0x6C, 0xF6, 0xF9, 0xE2,
            0x22, 0xB2, 0x25, 0xCA, 0xCC, 0xB8, 0xCC, 0x51, 0x83, 0x20, 0xAD, 0xB6, 0x23, 0xCB,
            0xF1, 0x94, 0x19, 0x48,
        ];
        let expected_iv: [u8; 16] = [
            0x7B, 0xD4, 0x28, 0x0E, 0x1A, 0x92, 0xFC, 0x49, 0xD0, 0x60, 0x43, 0x9C, 0xC1, 0xF8,
            0x08, 0x1C,
        ];

        assert_eq!(
            key, expected_key,
            "Key must match OpenSSL EVP_BytesToKey output"
        );
        assert_eq!(
            iv, expected_iv,
            "IV must match OpenSSL EVP_BytesToKey output"
        );
    }
    ...
    fn test_aes_encrypt_empty_plaintext() {
        ...
        assert!(
            !encrypted.is_empty(),
            "Encrypted empty plaintext should not be empty (padding)"
        );
        assert_eq!(
            encrypted.len(),
            16,
            "Encrypted empty plaintext should be one block (16 bytes)"
        );
    }
    ...
    fn test_aes_encrypt_various_lengths() {
        ...
        for len in [1, 15, 16, 17, 31, 32, 100] {
            ...
            assert_eq!(
                encrypted.len() % 16,
                0,
                "Ciphertext should be multiple of block size"
            );
        }
    }
    ...
    fn test_make_lnurl_different_amounts() {
        ...
        for amount in [1, 100, 1000, 10000, 1000000] {
            ...
            assert!(
                lnurl.starts_with("LNURL"),
                "LNURL should start with 'LNURL' for amount {}",
                amount
            );
            assert!(
                lnurl.len() > 50,
                "LNURL should have reasonable length for amount {}",
                amount
            );
        }
    }
    ...
    fn test_make_lnurl_different_secrets() {
        ...
        for secret in [
            "secret1",
            "secret2",
            "very_long_secret_with_special_chars!@#$%",
        ] {
            let lnurl = make_lnurl(base_url, secret, amount);
            assert!(
                lnurl.starts_with("LNURL"),
                "LNURL should start with 'LNURL' for secret"
            );
    ...
    fn test_make_lnurl_different_base_urls() {
        ...
        for base_url in [
            ...
            assert!(
                lnurl.starts_with("LNURL"),
                "LNURL should start with 'LNURL' for URL {}",
                base_url
            );
        }
    }
    ...
    fn test_make_lnurl_zero_amount() {
        ...
        assert!(
            lnurl.starts_with("LNURL"),
            "LNURL should work with zero amount"
        );
    }
    ...
    fn test_make_lnurl_max_amount() {
        ...
        assert!(
            lnurl.starts_with("LNURL"),
            "LNURL should work with max u64 amount"
        );
    }
    ...
    fn test_make_lnurl_empty_secret() {
        ...
        assert!(
            lnurl.starts_with("LNURL"),
            "LNURL should work with empty secret"
        );
    }
    ...
    fn test_make_lnurl_bech32_validity() {
        ...
        let url = decode_lnurl_to_url(&lnurl);
        assert!(url.starts_with(base_url), "decoded URL should contain base URL");
    }
    ...
    fn test_salted_header_presence() {
        ...
        let url = decode_lnurl_to_url(&lnurl);
        assert!(url.contains("?atm=1&p="), "URL should contain atm=1&p= parameter");
        let encoded_payload = url.split("&p=").nth(1).expect("Should have p parameter");
        let decoded_payload = URL_SAFE
            .decode(encoded_payload)
            .expect("Valid base64");

        // Check for "Salted__" header
        assert!(
            decoded_payload.len() >= 8,
            "Payload should be at least 8 bytes"
        );
        assert_eq!(
            &decoded_payload[0..8],
            b"Salted__",
            "Should start with Salted__ header"
        );
    }
    ...
    fn test_payload_structure() {
        ...
        assert!(
            decoded_payload.len() >= 8 + 8 + 16,
            "Payload should be at least 32 bytes (header + salt + encrypted block)"
        );
    }

    #[test]
    fn test_payload_length_divisible_by_22() {
        // LNBits Fossa backend requires: len(payload) % 22 == 0
        let base_url = "https://example.com/withdraw";
        let secret = "test_secret";

        for amount in [1u64, 100, 1000, 10000, 100000, 1000000, 9999999999] {
            let lnurl = make_lnurl(base_url, secret, amount);
            ...
            assert_eq!(
                encoded_payload.len() % 22,
                0,
                "Base64 payload length must be divisible by 22 for LNBits Fossa \
                 compatibility (amount={}, len={})",
                amount,
                encoded_payload.len()
            );
        }
    ...
    #[test]
    fn test_full_roundtrip_decrypt() {
        use cbc::Decryptor;
        use cipher::BlockDecryptMut;
        type Aes256CbcDec = Decryptor<Aes256>;

        let base_url = "https://example.com/withdraw";
        let secret = "roundtrip_secret";
        let amount = 5000u64;
        ...
        assert!(url.contains("?atm=1&p="));

        // Extract and decode base64 payload
        let encoded_payload = url.split("&p=").nth(1).expect("Should have p parameter");
        let decoded = URL_SAFE.decode(encoded_payload).expect("Valid base64");

        // Verify Salted__ header and extract salt + ciphertext
        assert_eq!(&decoded[0..8], b"Salted__");
        let salt: [u8; 8] = decoded[8..16].try_into().expect("8-byte salt");
        let ciphertext = &decoded[16..];

        // Derive key and IV from the same secret + extracted salt
        let mut key = [0u8; 32];
        let mut iv = [0u8; 16];
        derive_key_and_iv(secret, &salt, &mut key, &mut iv);

        // Decrypt
        let decryptor = Aes256CbcDec::new(key.as_ref().into(), iv.as_ref().into());
        let plaintext = decryptor
            .decrypt_padded_vec_mut::<cipher::block_padding::Pkcs7>(ciphertext)
            .expect("Decryption failed");
        let plaintext_str = String::from_utf8(plaintext).expect("Valid UTF-8");

        // Verify "PIN:amount" format
        let parts: Vec<&str> = plaintext_str.split(':').collect();
        assert_eq!(parts.len(), 2, "Plaintext should be PIN:amount");

        let pin: u16 = parts[0].parse().expect("PIN should be numeric");
        assert!((1000..10000).contains(&pin), "PIN should be 4 digits");

        let decoded_amount: u64 = parts[1].parse().expect("Amount should be numeric");
        assert_eq!(decoded_amount, amount, "Decoded amount must match input");
    }

    #[test]
    fn test_url_contains_atm_parameter() {
        let base_url = "https://example.com/withdraw";
        let secret = "test_secret";
        let amount = 1000u64;
        ...
        let url = decode_lnurl_to_url(&lnurl);
        assert!(
            url.contains("?atm=1&p="),
            "URL must contain ?atm=1&p= for LNBits Fossa compatibility, got: {}",
            url
        );
    }

    #[test]
    fn test_aes_encrypt_roundtrip() {
        use cbc::Decryptor;
        use cipher::BlockDecryptMut;
        type Aes256CbcDec = Decryptor<Aes256>;
